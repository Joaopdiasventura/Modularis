...
export class AppConfig {
  public static load(): AppConfigType {
    return {
      serviceName: read('MODULARIS_SERVICE_NAME', 'onboarding-service'),
      ...
      rabbitmq: {
        ...
        commandQueue: read(
          'MODULARIS_ONBOARDING_COMMAND_QUEUE',
          'onboarding-service.command.account-create',
        ),
        deliveryLimit: readNumber('MODULARIS_RABBITMQ_DELIVERY_LIMIT', 5),
      },
      postgres: {
        connectionString: read(
          'MODULARIS_POSTGRES_URL',
          'postgresql://modularis:modularis@localhost:5432/modularis_onboarding',
        ),
        connectionTimeoutMs: readNumber(
          'MODULARIS_POSTGRES_CONNECTION_TIMEOUT_MS',
          5000,
        ),
      },
      auth: {
